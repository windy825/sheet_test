...
# - Cross-checked matching (soft contract id, amount/date tolerance, token sim, combo)
# - Safe sorting and empty-state guards
# Run:
...
if not view.empty and owner_filter:
    ...
    if owners:
        view = view[view["담당자"].str.lower().isin(owners)] if "담당자" in view.columns else view
...
if not view.empty:
    ...
    fallback = sort_opt if sort_opt in view.columns else "계약ID"
    try:
...
contract_ids = view["계약ID"].tolist() if not view.empty else []
...
if sel and sel != "(미선택)":
    ...
    with st.expander("ℹ️ 개념 도움말", expanded=False):
        st.markdown("- **선수금**: 고객에게서 미리 받은 돈\n- **선급금**: 협력사에 미리 준 돈\n- **Gap**: 선수−선급 (양수=유리)")
    ...
    with t1:
        ...
        if df_r.empty:
            ...
            r_plot = detail[detail["direction"]=="선수금"][['date','amount']].dropna().copy()
            if not r_plot.empty:
                r_plot['month'] = pd.to_datetime(r_plot['date']).dt.to_period('M').dt.to_timestamp()
                st.bar_chart(r_plot.groupby('month')['amount'].sum())
    ...
    with t2:
        ...
        if df_a.empty:
            ...
            a_plot = detail[detail["direction"]=="선급금"][['date','amount']].dropna().copy()
            if not a_plot.empty:
                a_plot['month'] = pd.to_datetime(a_plot['date']).dt.to_period('M').dt.to_timestamp()
                st.bar_chart(a_plot.groupby('month')['amount'].sum())
